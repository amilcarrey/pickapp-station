...
const GrabCode: FC<GrabCodeProps> = ({ open, setOpen }) => {
  ...
  const numericUpdate = (e: number | string) => {
    const deleteLast = (setValue: Dispatch<SetStateAction<number | undefined>>) => {
      setValue((prev) => {
    ...
    const concatValue = (setValue: Dispatch<SetStateAction<number | undefined>>, e: number) => {
      setValue((prev) => {
        if (prev === undefined) {
          return Number(e)
        } else {
  ...
  return (
    <Transition.Root show={open} as={Fragment}>
      <Dialog as="div" className="relative z-10" initialFocus={cuiInputnRef} onClose={setOpen}>
        <Transition.Child
          as={Fragment}
              ...
              <Dialog.Panel className="relative  transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                <div>
                  <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full">
                    <UserIcon className="h-6 w-6 text-[#ff9900]" aria-hidden="true" />
                  </div>
                  <div className="text-center">
                    <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-[#2d6a3b]">
                      Ingresar
                    </Dialog.Title>
                    <div className="flex justify-center items-center gap-4 mt-2">
                      <div className='h-full flex flex-col gap-4 mt-2'>
                        <div className='flex gap-4 justify-between'>
                          ...
                          <input readOnly autoComplete="off" ref={claveInputRef} value={code} type="password" name="clave" placeholder='Código' className='border-2 border-black/25  rounded-xl px-4 py-1' />
                        </div>
                      </div>

                      <div id='keyboard' className='w-full grid grid-cols-3 gap-2'>
                        {numbers.map((number, index) => (
                          <button onClick={() => numericUpdate(number)} key={index} className='border-2 border-black/25  rounded-xl px-4 py-1'>{number}</button>
                        ))}
                      </div>
